...
<div x-data="{ masterOpen: true, suratOpen: false }" class="min-h-screen flex bg-gray-50">
    ...
    <main class="flex-1 p-8">
        <h1 class="text-2xl font-bold mb-4 text-gray-800">Edit Fakultas</h1>

        <form action="{{ route('fakultas.update', $fakultas->id_fakultas) }}" method="POST" class="space-y-4 bg-white shadow-md rounded-lg p-6 border border-gray-200">
            @csrf
            @method('PUT')

            <div>
                <label class="block font-medium text-gray-700">ID Fakultas</label>
                <input type="text" value="{{ $fakultas->id_fakultas }}" disabled class="border border-gray-300 p-2 rounded w-full bg-gray-100">
            </div>

            <div>
                <label class="block font-medium text-gray-700">Nama Fakultas</label>
                <input type="text" name="nama_fakultas" value="{{ $fakultas->nama_fakultas }}" class="border border-gray-300 p-2 rounded w-full focus:ring-2 focus:ring-amber-300 focus:outline-none" required>
            </div>
