        ...
        <tbody>
            @foreach($pejabat as $item)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('admin_akademik.pejabat.edit', $item->id) }}" class="btn btn-warning btn-sm">Edit</a>
                        <form action="{{ route('admin_akademik.pejabat.destroy', $item->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus pejabat ini?')">Hapus</button>
                        </form>
                    </td>
                </tr>
            @endforeach
