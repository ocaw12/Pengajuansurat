...
<div x-data="{ masterOpen: true, suratOpen: false }" class="min-h-screen flex bg-gray-50">
    ...
    <main class="flex-1 p-8">
        <h1 class="text-2xl font-bold mb-6 text-gray-800">Edit Prodi</h1>

        <form action="{{ route('prodi.update', $prodi->id_prodi) }}" method="POST" class="space-y-4 bg-white p-6 rounded-lg shadow">
            @csrf
            @method('PUT')

            <div>
                <label class="block font-medium text-gray-700">ID Prodi</label>
                <input type="text" value="{{ $prodi->id_prodi }}" disabled
            ...
            <div>
                ...
                <input type="text" name="nama_prodi" value="{{ $prodi->nama_prodi }}"
                       class="border p-2 rounded w-full focus:ring-2 focus:ring-amber-300" required>
            </div>

            <div>
                <label class="block font-medium text-gray-700">Email Prodi</label>
                <input type="email" name="email_prodi" value="{{ $prodi->email_prodi }}"
                       class="border p-2 rounded w-full focus:ring-2 focus:ring-amber-300" required>
            </div>
                ...
                <select name="id_fakultas" class="border p-2 rounded w-full focus:ring-2 focus:ring-amber-300" required>
                    @foreach ($fakultas as $f)
                        <option value="{{ $f->id_fakultas }}" 
                            {{ $prodi->id_fakultas == $f->id_fakultas ? 'selected' : '' }}>
                            {{ $f->nama_fakultas }}
                        </option>
                    @endforeach
                </select>
